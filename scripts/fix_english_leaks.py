...
Regras estritas:
1. 'Dhamma' como realidade ontológica deve ser 'realidade' (nunca 'fenômeno').
2. 'Bhavaṅga' não é 'subconsciente' (mantenha bhavaṅga).
3. 'Citta' (consciência), 'Cetasika' (fator mental), 'Kamma' (nunca karma).
4. O texto deve ser PT-BR culto, sem gerúndios de Portugal ('está a fazer'), usando 'está fazendo'.
5. Traduza O CONTEÚDO PARA PORTUGUÊS. O texto atual pode estar corrompido com pedaços em inglês.
...
for path, seg_id in leaks:
    ...
    for seg in data:
        if str(seg["id"]) == str(seg_id):
            ...
            if not source_text:
                print(f"Skipping {path}:{seg_id} (no source text)")
                continue
