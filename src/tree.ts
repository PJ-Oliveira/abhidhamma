...
import type { Manifest, WorkEntry } from "./types.js";

const GROUP_ORDER = ["abhidhamma", "outros", "visuddhimagga", "comentarios"] as const;

export type SelectHandler = (workId: string, partKey: string, chunkIndex: number, segId: number | null) => void;

export function renderTree(manifest: Manifest, container: HTMLElement, onSelect: SelectHandler): void {
  container.innerHTML = "";
  const root = document.createElement("ul");

  for (const group of GROUP_ORDER) {
    const works = manifest.groups[group];
    if (!works || !works.length) continue;
...
function buildWorkNode(work: WorkEntry, onSelect: SelectHandler): HTMLLIElement {
...
export function markActiveLeaf(container: HTMLElement, workId: string, partKey: string): void {
  container.querySelectorAll(".node-label.leaf-active").forEach((el) => {
    el.classList.remove("leaf-active");
  });
  const el = container.querySelector<HTMLElement>(
    `.node-label.leaf[data-work-id="${workId}"][data-part-key="${partKey}"]`
  );
  if (!el) return;
