...
import type { Manifest, Segment, WorkEntry, TocEntry } from "./types.js";
...
let glossaryData: GlossEntry[] | null = null;

async function loadGlossary(): Promise<GlossEntry[]> {
  if (glossaryData) return glossaryData;
  try {
    const res = await fetch("data/dictionary/common_pali.json");
    if (!res.ok) return [];
    const data = await res.json();
    glossaryData = (data.entries ?? []) as GlossEntry[];
    return glossaryData;
  } catch {
    return [];
  }
...
export function collectUsedTerms(segs: Segment[], glossary: GlossEntry[]): GlossEntry[] {
...
export function buildGlossaryHtml(terms: GlossEntry[], langMode: LangMode): string {
  if (terms.length === 0) return "";
  const mode = langMode as string;
  const showEn = mode.includes("en");
  const showPt = mode.includes("pt");
  const showEs = mode.includes("es");
  const rows = terms.map((term) => {
    const parts: string[] = [`<strong>${escHtml(term.h)}</strong>`];
    if (term.pos) parts.push(`<em>(${escHtml(term.pos)})</em>`);
    if (showEn || (!showPt && !showEs)) parts.push(escHtml(term.en));
    if (showPt) parts.push(escHtml(term.pt));
    if (showEs && term.es) parts.push(escHtml(term.es));
...
export function buildZip(entries: ZipEntry[]): Uint8Array {
  ...
  const fileData: Array<{ name: Uint8Array; data: Uint8Array; crc: number; offset: number }> = [];
  ...
  for (const { name, data, crc, offset: fileOffset } of fileData) {
    ...
    writeUint32LE(cv, 0, 0x02014b50);  // central dir signature
    writeUint16LE(cv, 4, 20);           // version made
    writeUint16LE(cv, 6, 20);           // version needed
    writeUint16LE(cv, 8, 0);            // flags
    writeUint16LE(cv, 10, 0);           // compression: store
  ...
  writeUint32LE(ev, 0, 0x06054b50);  // EOCD signature
...
// Walk a DOM node and produce safe export HTML.
// Converts <b>/<i>/<sup> to their semantic equivalents,
// converts <p rend="..."> verse markers to <br> + class spans,
// and strips any unknown tags (keeping their text content).
export function linkifyText(text: string, termsMap?: Map<string, string>): string {
  if (!text) return "";
  if (!termsMap || termsMap.size === 0) return escHtml(text);
...
export function walkNode(node: Node, termsMap?: Map<string, string>): string {
  if (node.nodeType === Node.TEXT_NODE) return linkifyText(node.textContent ?? "", termsMap);
...
// Convert a segment field value (which may contain inline HTML) to safe export HTML.
export function fieldHtml(raw: string, termsMap?: Map<string, string>): string {
  if (!raw) return "";
  if (!raw.includes("<")) return linkifyText(raw, termsMap);
  const parser = new DOMParser();
  const doc = parser.parseFromString(`<body>${raw}</body>`, "text/html");
  return walkNode(doc.body, termsMap);
}

export function segToHtml(seg: Segment, langMode: LangMode, termsMap?: Map<string, string>): string {
  const rend = seg.rend;
  ...
  // Note: we do NOT linkify the Pali source lines, only the translations
  ...
  if (rend === "chapter") return `<h2 class="seg chapter">${inner}</h2>`;
  if (rend === "subhead") return `<h3 class="seg subhead">${inner}</h3>`;
  if (rend === "subsubhead") return `<h4 class="seg subsubhead">${inner}</h4>`;
  if (rend === "hangnum") return `<p class="seg hangnum">${inner}</p>`;
  if (rend === "indent") return `<blockquote class="seg indent">${inner}</blockquote>`;
  if (rend === "footnote") return `<p class="seg footnote">${inner}</p>`;
  if (rend === "centre") return `<p class="seg centre">${inner}</p>`;
  if (rend === "glossary") return `<p class="seg glossary">${inner}</p>`;
...
// ── PDF: open new window (blob URL) and print ────────────────────────────────
...
function openPrintWindow(html: string, title: string): void {
  ...
  if (!w) {
    // popup blocked — download the HTML so the user can open and Ctrl+P it
...
// ── Full Corpus EPUB builder (multi-chapter, hierarchical TOC) ───────────────
...
async function buildFullCorpusEpub(
  manifest: Manifest,
  langMode: LangMode,
  ...
  rawGlossary.forEach((entry, idx) => {
    // Basic headword mapped to dict_{idx}
  ...
  for (const [group, works] of Object.entries(manifest.groups)) {
    for (const work of works) {
      ...
      await new Promise(r => setTimeout(r, 10)); // Yield thread to update UI

      // Fetch ALL segments for this work (all parts)
      ...
      for (const part of Object.values(work.parts)) {
        for (const file of part.files) {
          try {
            ...
            if (res.ok) {
              const segs = (await res.json()) as Segment[];
              allSegs.push(...segs);
            }
      ...
      // Build body HTML with anchor IDs on chapters
      ...
      for (const seg of allSegs) {
        ...
        if (rend === "chapter" || rend === "book" || rend === "title" || rend === "nikaya") {
          ...
          if (label) anchors.push({ id: anchorId, label });
          // Inject anchor ID into the heading tag
...
${bodyParts.join("\n")}
  ...
  rawGlossary.forEach((term, idx) => {
    ...
    if (term.es) parts.push(escHtml(term.es));
...
${glossaryRows}
  ...
  const manifestItems = chapters.map(
    (c) => `  <item id="${c.fileId}" href="${c.href}" media-type="application/xhtml+xml"/>`
  ).join("\n");
  const spineItems = chapters.map(
    (c) => `  <itemref idref="${c.fileId}"/>`
  ).join("\n");
...
${spineItems}
  ...
  // ── Build hierarchical NCX ──
  let playOrder = 0;
  const ncxPoints: string[] = [];
  const groupMap = new Map<string, CorpusChapter[]>();
...
<rootfiles>
  ...
  if (fontReg.length > 0) zipEntries.push({ name: "OEBPS/fonts/GentiumBookPlus-Regular.ttf", data: fontReg });
...
async function fetchWorkSegments(
  workId: string,
  files: string[],
  fromSegId: number | null,
  toSegId: number | null
): Promise<Segment[]> {
  const all: Segment[] = [];
  ...
  if (fromSegId == null && toSegId == null) return all;
  return all.filter(
    (s) =>
      (fromSegId == null || s.id >= fromSegId) &&
...
function makeButton(text: string, id: string): HTMLButtonElement {
...
export function initExportPanel(
  ...
  const allWorks: Array<{ group: string; work: WorkEntry }> = [];
  for (const [group, works] of Object.entries(manifest.groups)) {
    for (const work of works) allWorks.push({ group, work });
  }
  ...
  const langOptions: Array<[LangMode, string]> = [
  ...
  function getSelectedWork(): WorkEntry | null {
    const id = workSel.value;
    return allWorks.find((w) => w.work.id === id)?.work ?? null;
  }

  function getSelectedPartKeys(): string[] {
    return Array.from(partsDiv.querySelectorAll<HTMLInputElement>("input[type=checkbox]:checked")).map(
      (cb) => cb.value
    );
  }

  function getChapterToc(work: WorkEntry, partKey: string): TocEntry[] {
    return (work.parts[partKey]?.toc ?? []).filter((e) => e.rend === "chapter");
  }

  function refreshChapterRange(work: WorkEntry, partKeys: string[]): void {
  ...
  function refreshParts(work: WorkEntry): void {
    partsDiv.innerHTML = "";
  ...
  workSel.addEventListener("change", () => {
    ...
    if (!isAll) {
      ...
      if (work) refreshParts(work);
    }
  });

  // Seed with current work if available, else first
  const currentId = getCurrentWorkId();
  if (currentId) {
    const opt = Array.from(workSel.options).find((o) => o.value === currentId);
    if (opt) workSel.value = currentId;
  }
  ...
  async function runExport(format: "pdf" | "epub"): Promise<void> {
    const langMode = langSel.value as LangMode;
    ...
    // Determine chapter range only when single part
    let fromSegId: number | null = null;
    let toSegId: number | null = null;
    if (partKeys.length === 1 && chapDiv.style.display !== "none") {
      const fromVal = chapFromSel.value;
      const toVal = chapToSel.value;
      if (fromVal) fromSegId = Number(fromVal);
      if (toVal) {
        // toSegId = last segment of selected chapter = id of NEXT chapter entry - 1
    ...
    try {
      // Fetch segments for all selected parts
      let allSegs: Segment[] = [];
      let partLabel = "";
      for (const partKey of partKeys) {
        ...
        const segs = await fetchWorkSegments(
          work.id,
          part.files,
          partKeys.length === 1 ? fromSegId : null,
          partKeys.length === 1 ? toSegId : null
        );
      ...
      // Load glossary and build appendix
