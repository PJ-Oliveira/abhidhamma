...
// Define the tools for the LLM to use to query the ontology
const ontologyTools = [
  {
    type: "function",
    function: {
...
// Handle initialization and messaging
self.onmessage = async (event: MessageEvent) => {
  ...
  if (type === 'INITIALIZE') {
    ...
    try {
      const initProgressCallback: InitProgressCallback = (initProgress: any) => {
        self.postMessage({ type: 'PROGRESS', payload: initProgress });
      };

      // Default model, can be overridden by the payload
    ...
    try {
      // payload.messages should include the original query, the assistant's tool call, and the tool response
      const messages = [
        { role: 'system', content: SYSTEM_PROMPT },
        ...payload.messages 
      ];
