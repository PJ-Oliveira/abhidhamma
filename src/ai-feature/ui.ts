...
export function initInterlocutorPanel(container: HTMLElement): void {
    ...
    wrapper.innerHTML = `
        ...
        <select id="scenario-select" style="box-sizing: border-box; padding: 8px; border-radius: 4px; border: 1px solid var(--border, #d9c3a6); width: 100%; background: var(--bg-content, #f8f0e5); color: var(--text, #33261a);">
            ...
            ${CanonicalScenarios.map(s => '<option value="' + s.id + '">' + s.source + ': ' + (s.title as any)[settings.uiLang] + '</option>').join('')}
    ...
    // Setup events
    const select = wrapper.querySelector('#scenario-select') as HTMLSelectElement;
    const claimDisplay = wrapper.querySelector('#claim-display') as HTMLElement;
    const claimText = wrapper.querySelector('#claim-text') as HTMLElement;
            const submitBtn = wrapper.querySelector('#interlocutor-submit') as HTMLButtonElement;
    
    const elaborateBtn = wrapper.querySelector('#btn-elaborate') as HTMLButtonElement;
    const pedagogicalDisplay = wrapper.querySelector('#pedagogical-display') as HTMLElement;

    elaborateBtn.addEventListener('click', () => {
        pedagogicalDisplay.style.display = 'block';
        elaborateBtn.style.display = 'none'; // Esconde o botão após clicar
    });
    ...
    select.addEventListener('change', () => {
        ...
        if (val) {
            ...
            if (scenario) {
                claimText.textContent = '"' + (scenario.claimText as any)[settings.uiLang] + '"';
        ...
        // Reset refutation view
        (wrapper.querySelector('#refutation-display') as HTMLElement).style.display = 'none';
    ...
    submitBtn.addEventListener('click', () => {
        ...
        const scenario = CanonicalScenarios.find(s => s.id === val);
        if (scenario) {
            claim = (scenario.claimText as any)[settings.uiLang];
        }
        ...
        if (claim) {
            ...
            // Simular a latência do WebWorker/LLM e exibir a refutação detalhada
            setTimeout(() => {
                submitBtn.textContent = origText;
                submitBtn.disabled = false;

                const refutationDisplay = wrapper.querySelector('#refutation-display') as HTMLElement;
                const verdictEl = wrapper.querySelector('#refutation-verdict') as HTMLElement;
                const historyEl = wrapper.querySelector('#refutation-history') as HTMLElement;
                const metaLogicEl = wrapper.querySelector('#refutation-metalogic') as HTMLElement;
                const paradoxEl = wrapper.querySelector('#refutation-paradox') as HTMLElement;
                const logicEl = wrapper.querySelector('#refutation-logic') as HTMLElement;
                
                const pedaAnalogyEl = wrapper.querySelector('#pedagogical-analogy') as HTMLElement;
                const pedaBreakdownEl = wrapper.querySelector('#pedagogical-breakdown') as HTMLElement;
                ...
                pedagogicalDisplay.style.display = 'none'; // Esconde por padrão no novo submit
