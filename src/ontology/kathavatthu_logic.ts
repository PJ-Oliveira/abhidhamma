...
export enum KathavatthuRule {
    ANULOMA = 'Anuloma (Forward progression)',
    PATIKAMMA = 'Paṭikamma (Counter-argument)',
    NIGGAHA = 'Niggaha (Refutation)',
    UPANAYANA = 'Upanayana (Application)',
    NITTHANA = 'Niṭṭhāna (Conclusion)',
    PACCANIKA = 'Paccanīka (Contrary / Inverse)'
}

export function evaluateCustomClaim(claimText: string): any {
    const text = claimText.toLowerCase();
    
    // Evaluate if it relates to "Self", "Person", "Soul", "I"
...
5. <strong>${KathavatthuRule.NITTHANA}:</strong> The proposition is false.
    ...
    // Evaluate if it relates to "Time", "Past", "Future"
...
5. <strong>${KathavatthuRule.NITTHANA}:</strong> The proposition is false.
    ...
    // Evaluate if it relates to "Citta", "Consciousness", "Transmigration", "Momentary"
...
5. <strong>${KathavatthuRule.NITTHANA}:</strong> The proposition is false. Citta is a rapid succession of momentary (khaṇika) events.
    ...
    // Evaluate if it relates to "Sabhava", "Lakkhana", "Specific vs General Characteristics"
...
5. <strong>${KathavatthuRule.NITTHANA}:</strong> The proposition is logically flawed.
    ...
    // Evaluate if it relates to "Permanent Cause", "Basis of Happiness", "Simultaneous Origination"
...
export function evaluateScenario(scenarioId: string, lang: 'en'|'pt'|'es' = 'en'): any {
    const scenario = CanonicalScenarios.find(s => s.id === scenarioId);
    if (!scenario) return evaluateCustomClaim("");

    // Helper for extracting potentially localized strings
    const getL = (obj: any, defaultText: string) => {
    ...
    // We import t from i18n but since it's an ontology file it's better to avoid UI dependencies.
    // So we just localize here.
