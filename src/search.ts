...
import type { SearchHit, SearchManifest, SearchShard } from "./types.js";
...
async function ensureManifest(): Promise<void> {
  ...
  try {
    ...
    shardManifest = (await res.json()) as SearchManifest;
  } catch (err) {
...
function shardKeyFor(token: string): string {
  const ch = token[0] ?? "";
  return /[a-z]/i.test(ch) ? ch.toLowerCase() : "misc";
}

async function loadShard(key: string): Promise<SearchShard | null> {
  ...
  const data = res.ok ? ((await res.json()) as SearchShard) : null;
...
async function searchQuery(query: string): Promise<SearchHit[]> {
  ...
  const matchedIndexes = new Set<number>(shard.postings[token] ?? []);
  if (matchedIndexes.size < MAX_RESULTS) {
    for (const [candidate, postings] of Object.entries(shard.postings)) {
      ...
      for (const idx of postings) matchedIndexes.add(idx);
      if (matchedIndexes.size >= MAX_RESULTS) break;
    }
  }
